...
class DomainAbstractedTask : public tasks::DelegatingTask {
    ...
    std::vector<std::vector<Bitset>> inverse_value_map;
    ...
    // Note: This is copied from sdscp
    void build_inverse_value_map();
...
public:
    ...
    const std::vector<std::vector<Bitset>> &get_inverse_value_map() {
        if (inverse_value_map.empty()) {
            build_inverse_value_map();
        }
        return inverse_value_map;
    }

    // Transformation to concrete_values is more costly than from concrete to abstract.
    // Note: This is copied from sdscp
    void get_concrete_values(int var_index, const Bitset &abstract_values, Bitset &concrete_values);

    // Transformation to the first concrete value of the given fact.
    int get_concrete_value(const FactPair &fact);
};
}
