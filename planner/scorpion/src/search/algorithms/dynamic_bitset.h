...
public:
    ...
    DynamicBitset& operator=(const DynamicBitset &other) {
        if (this != &other) {
            // the the size respective to other
            num_bits = other.size();
            blocks = std::vector<Block>(compute_num_blocks(other.size()), zeros);
            // set bits correctly
            *this |= other;
        }
        return *this;
    ...
    */
    ...
    DynamicBitset& operator&=(const DynamicBitset &rhs) {
        assert(size() == rhs.size());
        for (std::size_t i = 0; i < blocks.size(); ++i) {
            // std::cout << blocks[i] << " " << rhs.blocks[i] << std::endl;
            blocks[i] &= rhs.blocks[i];
        }
        return *this;
    }
    ...
    bool is_disjunct(const DynamicBitset &other) const {
        assert(size() == other.size());
        for (std::size_t i = 0; i < blocks.size(); ++i) {
            if (blocks[i] & other.blocks[i])
                return false;
    ...
    friend std::ostream &operator<<(
      std::ostream &os, const DynamicBitset &bitset) {
        os << bitset.size() << "={";
