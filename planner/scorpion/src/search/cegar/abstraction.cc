...
SplitTree Abstraction::extract_split_tree() {
...
pair<int, int> Abstraction::refine(
    ...
    pair<NodeID, NodeID> context_split_node_ids = split_tree.split(
        ...
        v1_cartesian_set.get_bitset_from_var(var),
        v2_cartesian_set.get_bitset_from_var(var));

    unique_ptr<AbstractState> v1 = utils::make_unique_ptr<AbstractState>(
        v1_id, node_ids.first, context_split_node_ids.first, move(v1_cartesian_set));
    unique_ptr<AbstractState> v2 = utils::make_unique_ptr<AbstractState>(
        v2_id, node_ids.second, context_split_node_ids.second, move(v2_cartesian_set));
