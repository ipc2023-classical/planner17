...
SaturatedCostPartitioningOnlineHeuristic::SaturatedCostPartitioningOnlineHeuristic(
    ...
    : Heuristic(opts),
      ...
      remaining_tcf(RemainingTransitionCostFunction(task_info, bdd_builder)),
    ...
    if (improve_heuristic && (num_evaluated_states % interval == 0)) {
        ...
        if (saturator == Saturator::PERIMSTAR) {
            ...
            cost_partitioning = compute_perim_saturated_cost_partitioning(
                abstractions, order, remaining_tcf, abstract_state_ids, max_num_transitions);
        } else {
            cost_partitioning = cp_function(abstractions, order, remaining_tcf, abstract_state_ids, max_num_transitions);
        }
        ++num_scps_computed;
...
static shared_ptr<Heuristic> _parse(OptionParser &parser) {
    ...
    shared_ptr<cost_saturation::TaskInfo> task_info =
        make_shared<cost_saturation::TaskInfo>(TaskProxy(*task));
    ...
    return make_shared<SaturatedCostPartitioningOnlineHeuristic>(
        opts, move(abstractions), task_info, bdd_builder, move(dead_ends), max_num_transitions);
}

static Plugin<Evaluator> _plugin("stcp_online", _parse, "heuristics_cost_partitioning");
}
