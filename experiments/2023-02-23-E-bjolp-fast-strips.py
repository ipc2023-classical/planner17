...
CONFIGS = [
    (f"sys-scp-{pattern_type}-60s-{cartesian_nick}-bjolp-fast", [
        "--evaluator",
    ...
    (f"bjolp-scp-fast", ["--evaluator", f"lmc=lmcount(lm_merged([lm_rhw(),lm_hm(m=1)]), admissible=true, cost_partitioning=suboptimal, greedy=true, reuse_costs=true, scoring_function=max_heuristic_per_stolen_costs)", "--search", "astar(lmc,lazy_evaluator=lmc, pruning=limited_pruning(pruning=atom_centric_stubborn_sets(), min_required_pruning_ratio=0.2))"]),
]
