...
OFFLINE_ORDER_TIME = 240 if project.REMOTE else 5
...
CONFIGS = [
    (
        ...
        [
            "--search",
            f"astar(operatorcounting([pho_abstraction_constraints([{ABSTRACTIONS}],strategy={spho_config.strategy})],cache_lp={spho_config.cache},debug_cache={spho_config.debug}),verbosity=silent)",
        ],
    )
    for spho_config in [
    ...
    (
        ...
        [
            "--search",
            f"astar(pho_online([{ABSTRACTIONS}],interval=10K),verbosity=silent)",
        ],
    ),
    (
        "pho_offline",
        [
            "--search",
            f"astar(pho_offline([{ABSTRACTIONS}],max_time={OFFLINE_ORDER_TIME},max_optimization_time=0),verbosity=silent)",
        ],
    ),
]
