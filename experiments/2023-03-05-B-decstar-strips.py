...
CONFIGS = [
    ("fork-optimized",  ["--decoupling", "fork(search_type=asda, pruning=dominance(irrelevance=TRANSITIONS), max_leaf_size=100000)",
                         "--search", "astar(lmcut, pruning_heuristic=lmcut(search_type=STAR), pruning=stubborn_sets_decoupled(min_pruning_ratio=0.25, special_case_optimizations=true))"]),
    ("ds-sss-oss", ["--decoupling", "lp_general(factoring_time_limit=30, memory_limit=7000, add_cg_sccs=true, strategy=mm_approx, min_flexibility=0.8)", 
                           "--search", "astar(lmcut, "
    ...
    ("ds", ["--decoupling", "lp_general(factoring_time_limit=30, memory_limit=7000, add_cg_sccs=true, strategy=mm_approx, min_flexibility=0.8)", 
            "--search", "astar(lmcut)"]),
]
