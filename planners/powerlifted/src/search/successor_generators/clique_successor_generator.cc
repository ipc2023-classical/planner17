                                                   ...
                                                   const Assignment &second_assignment)
{
    ...
    for (const auto &literal : literals) {
        ...
        if (!empty_assignment) {
            ...
            // A positive literal is inconsistent if there does not exist a (partially ground) atom in the state.
            // A negative literal is inconsistent if the assignment matches a fully ground atom in the state.

            if (!literal.is_negated() && !consistent_with_state) {
                return false;
            }
            else if (literal.is_negated() && consistent_with_state && ((predicate_arity == 1) || ((predicate_arity == 2) && (second_position >= 0)))) {
                return false;
            }
        }
    }
    ...
    for (const auto &relation : relations) {
        ...
        for (const auto &atom : relation.tuples) {
            for (size_t first_position = 0; first_position < atom.size(); ++first_position) {
                ...
                assignment_set[get_rank(first_position, first_object, -1, -1, predicate_arity, num_objects)] = true;

                for (size_t second_position = first_position + 1; second_position < atom.size(); ++second_position) {
                    const auto second_object = atom[second_position];
                    assignment_set[get_rank(second_position, second_object, -1, -1, predicate_arity, num_objects)] = true;
                    assignment_set[get_rank(first_position, first_object, second_position, second_object, predicate_arity, num_objects)] = true;
                }
            }
        }
    }
...
CliqueSuccessorGenerator::CliqueSuccessorGenerator(const Task &task, const CliquePivot &pivot)
    : task(task), pivot(pivot), generated_total_(0), generated_applicable_(0)
{
    ...
    for (const auto &action : task.get_action_schemas()) {
        ...
        for (const auto &literal : literals) {
            ...
            else if (dynamic_predicate_names.find(literal.get_name()) != dynamic_predicate_names.end()) {
        ...
        for (size_t first_id = 0; first_id < to_vertex_assignment.size(); ++first_id) {
            for (size_t second_id = (first_id + 1); second_id < to_vertex_assignment.size(); ++second_id) {
                ...
                if (first_assignment.parameter_index != second_assignment.parameter_index) {
                    if (consistent_literals(static_precondition, first_assignment, second_assignment)) {
...
CliqueSuccessorGenerator::applicable_actions_nullary_case(const ActionSchema &action, const DBState &state)
{
    ...
    generated_total_ += applicable_actions.size();
    generated_applicable_ += applicable_actions.size();

    return applicable_actions;
}

vector<LiftedOperatorId>
CliqueSuccessorGenerator::applicable_actions_unary_case(const ActionSchema &action, const DBState &state)
{
    vector<LiftedOperatorId> applicable_actions;
...
CliqueSuccessorGenerator::applicable_actions_general_case(const ActionSchema &action, const DBState &state)
{
    ...
    const auto &statically_consistent_assignments = action_statically_consistent_assignments.find(action)->second;
    ...
    if (num_parameters == 2)
    {
        for (const auto &pair : statically_consistent_assignments) {
            if (consistent_literals(preconds.dynamic_preconds, pair.first_assignment, pair.second_assignment)) {
                cliques.push_back(std::vector<uint32_t>({ (uint32_t) pair.first_position, (uint32_t) pair.second_position }));
            }
        }
    }
    else
