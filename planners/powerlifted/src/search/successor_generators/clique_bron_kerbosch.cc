                                 ...
                                 vector<vector<uint32_t>> &out_cliques)
{
    ...
    while (!stack.empty()) {
        ...
        if ((r.size() + p.size()) < max_clique_size) {
            ...
            // const auto u = choose_pivot_min_difference(adjacency_list, p, x);
            ...
            while (p_i < p.size()) {
                ...
                if (v < w) {
                    ...
                    set_intersection(n_v.begin(), n_v.end(), p.begin(), p.end(), back_inserter(intersect_p));

                    vector<uint32_t> intersect_x;
                    set_intersection(n_v.begin(), n_v.end(), x.begin(), x.end(), back_inserter(intersect_x));

                    // BronKerbosch2(R ⋃ {v}, P ⋂ N(v), X ⋂ N(v))
                    stack.push_back(make_tuple(std::move(union_r), std::move(intersect_p), std::move(intersect_x)));

                    // P := P \ {v}
                    erase_index(p, p_i); // This also moves the p_i "forward".  // TODO: Iterate backwards to make this more efficient?
                                        ...
                                        vector<vector<uint32_t>> &out_cliques)
{
    ...
    for (auto vertex_id : degeneracy_ordering) {
        ...
        intersect_p.clear();
        set_intersection(vertex_neighbors.begin(), vertex_neighbors.end(), p.begin(), p.end(), back_inserter(intersect_p));

        intersect_x.clear();
        set_intersection(vertex_neighbors.begin(), vertex_neighbors.end(), x.begin(), x.end(), back_inserter(intersect_x));
...
struct FirstPivot {
    static uint32_t get_pivot(const vector<uint32_t> adjacency_list[], const vector<uint32_t> &p, const vector<uint32_t> &x)
    {
...
struct MaxNeighborhoodPivot {
    inline static uint32_t get_pivot(const vector<uint32_t> adjacency_list[], const vector<uint32_t> &p, const vector<uint32_t> &x)
    {
        // Find the vertex v that maximizes |n(v)|.

        bool initialized = false;
        uint32_t best_vertex = p[0];  // Needed to suppress error-maybe-uninitialized, p.size() > 0 always.
        size_t best_score;
...
void bron_kerbosch_first_pivot(const vector<uint32_t> adjacency_list[],
                               ...
                               vector<vector<uint32_t>> &out_cliques)
{
    bron_kerbosch_with_vertex_ordering<FirstPivot>(adjacency_list, vertices, max_clique_size, out_cliques);
}
...
void bron_kerbosch_max_neighborhood_pivot(const vector<uint32_t> adjacency_list[],
                                          ...
                                          vector<vector<uint32_t>> &out_cliques)
{
    bron_kerbosch_with_vertex_ordering<MaxNeighborhoodPivot>(adjacency_list, vertices, max_clique_size, out_cliques);
}
...
void bron_kerbosch_min_difference_pivot(const vector<uint32_t> adjacency_list[],
                                        ...
                                        vector<vector<uint32_t>> &out_cliques)
{
    bron_kerbosch_with_vertex_ordering<MinDifferencePivot>(adjacency_list, vertices, max_clique_size, out_cliques);
}
