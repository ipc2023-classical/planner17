...
ExplicitProjectionFactory::ExplicitProjectionFactory(
    ...
    : task_proxy(task_proxy),
      ...
      has_loop(task_proxy.get_operators().size(), false),
      has_outgoing(task_proxy.get_operators().size(), false),
      num_states(0),
      num_transitions(0),
      init_state_id(-1),
      num_transitions_by_operator(task_proxy.get_operators().size(), 0) {
    ...
    abstraction_function = utils::make_unique_ptr<ProjectionFunction>(pattern, move(hash_multipliers));
    init_state_id = abstraction_function->get_abstract_state_id(task_proxy.get_initial_state());

    compute_transitions();
    goal_states = compute_goal_states();
}

unordered_set<int> ExplicitProjectionFactory::compute_goal_states() const {
...
unique_ptr<Abstraction> ExplicitProjectionFactory::convert_to_abstraction() {
    return utils::make_unique_ptr<ExplicitAbstractionProjection>(
        task_info,
        bdd_builder,
