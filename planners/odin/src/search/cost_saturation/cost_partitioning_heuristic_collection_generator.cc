...
CostPartitioningHeuristicCollectionGenerator::generate_cost_partitionings(
    ...
    CostPartitioningHeuristic cp_for_init = cp_function(
        abstractions, order_for_init, remaining_tcf, abstract_state_ids_for_init, max_num_transitions);
    ...
    while (static_cast<int>(cp_heuristics.size()) < max_orders &&
        ...
        if (is_first_order) {
            ...
            cp_heuristic = cp_function(abstractions, order, remaining_tcf, abstract_state_ids, max_num_transitions);
        }
