...
AbstractState::AbstractState(
    int state_id, NodeID node_id, NodeID split_node_id, CartesianSet &&cartesian_set)
    : state_id(state_id),
      node_id(node_id),
...
NodeID AbstractState::get_split_node_id() const {
    return split_node_id;
}

unique_ptr<AbstractState> AbstractState::get_trivial_abstract_state(
    const vector<int> &domain_sizes) {
    return utils::make_unique_ptr<AbstractState>(0, 0, 0, CartesianSet(domain_sizes));
}
}
