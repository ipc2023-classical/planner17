...
std::vector<int> get_concrete_domain_sizes() {
    vector<int> domain_sizes;
    domain_sizes.reserve(tasks::g_root_task->get_num_variables());
    for (int var_id = 0; var_id < (int)tasks::g_root_task->get_num_variables(); ++var_id) {
        domain_sizes.push_back(tasks::g_root_task->get_variable_domain_size(var_id));
    }
    return domain_sizes;
}
