...
LeavesPermutation::LeavesPermutation(const Permutation &per)
    ...
    for(LeafFactorID factor(0); factor < g_leaves.size(); ++factor){
        for(LeafStateHash s(0); s < g_state_registry->size(factor); ++s) {
            ...
            LeafStateHash symm_id = ExplicitStateCPG::get_leaf_state_hash(symm_leaf_state, symm_factor);
...
cmp_t LeavesPermutation::cmp_num_states(const SymmetryCPG &cpg) const {
...
cmp_t LeavesPermutation::cmp_goal_cost(const SymmetryCPG &cpg) const {
    for (LeafFactorID to_factor : factors_affected){
        LeafFactorID from_factor = from_factors[to_factor];
        if (factors_affected.size() == 2 && from_factor <= to_factor) {
            // This has been compared in the other direction or is the same factor
...
cmp_t LeavesPermutation::cmp_prices(const SymmetryCPG &cpg) const {
    for (LeafFactorID to_factor : factors_affected){
        LeafFactorID from_factor = from_factors[to_factor];
        if(factors_affected.size() == 2 && from_factor < to_factor) {
...
bool CenterPermutation::is_symmetric_op(const Operator &op, const Operator &op2) const {
    const auto effects = op.get_effects(LeafFactorID::CENTER);
    const auto effects2 = op2.get_effects(LeafFactorID::CENTER);
...
bool DecoupledPermutation::replace_if_less(vector<int> &center, SymmetryCPG &leaves,
...
bool DecoupledPermutation::replace_if_less_leaves(SymmetryCPG &leaves,
                                                  const LexicographicOrdering &lex_ordering) const {
...
inline bool are_equal(OpsLeafProps<Condition> preconds1, OpsLeafProps<Condition> preconds2) {
...
inline bool are_equal(OpsLeafProps<Effect> effs1, OpsLeafProps<Effect> effs2) {
...
OperatorID DecoupledPermutation::find_matching_min_cost_permuted_op(const Operator &main_op,
    ...
    if (!center_permutation && !leaves_permutation){
        assert(find(ops.begin(), ops.end(), main_op.get_id()) != ops.end());
        return main_op.get_id();
    }
...
bool DecoupledPermutation::stabilizes_center(const GlobalState &center) const {
    if(!center_permutation){
        return true;
    }
