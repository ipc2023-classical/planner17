...
bool SymStateSpaceManager::is_relevant_op(const Operator & op) const {
...
void SymStateSpaceManager::all_image(const BDD &bdd, vector<BDD> &res, int nodeLimit) const {
    if(all_transitions.empty()) {
        ...
        merge(vars, all_transitions, mergeTR, p.max_tr_time, p.max_tr_size);
    }
...
void SymStateSpaceManager::all_preimage(const BDD &bdd, vector<BDD> &res, int nodeLimit) const {
    if(all_transitions.empty()) {
        for (const auto &trs : transitions) {
            for(const auto & tr : trs.second) {
                all_transitions.push_back(tr);
