...
class SymStateSpaceManager {
    ...
    void all_image(const BDD &bdd, std::vector<BDD> &res, int maxNodes) const;
    void all_preimage(const BDD &bdd, std::vector<BDD> &res, int maxNodes) const;
...
protected:
    ...
    mutable std::vector<TransitionRelation> all_transitions; //Used for breadth first search
    ...
    bool is_relevant_op(const Operator & op) const;
...
public:
    ...
    SymStateSpaceManager(SymVariables *v, const SymParamsMgr &params,
            const std::set<int> & relevant_vars_, BDD initialState_, BDD goal_) :
                SymStateSpaceManager(v, params, relevant_vars_) {
    ...
    virtual double stateCount(const Bucket &bucket) const {
        return vars->numStates(bucket);
    }

    virtual double stateCount(const BDD &bdd) const {
        return vars->numStates(bdd);
    }
    ...
    inline void all_image(bool fw,
                           const BDD &bdd, std::vector<BDD> &res,
                           int maxNodes) {
        if (fw) {
            all_image(bdd, res, maxNodes);
        } else {
            all_preimage(bdd, res, maxNodes);
        }
    }
