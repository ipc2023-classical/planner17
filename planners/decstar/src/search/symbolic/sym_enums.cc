...
namespace symbolic {
ostream &operator<<(ostream &os, const LinearPDBStrategy &st) {
    switch (st) {
    ...
    default:
        cerr << "Name of LinearPDBStrategy not known";
        exit_with(utils::ExitCode::SEARCH_UNSUPPORTED);
    }
}

ostream &operator<<(ostream &os, const RelaxDirStrategy &relaxDir) {
    switch (relaxDir) {
    ...
    case RelaxDirStrategy::SWITCHBACK:
        return os << "sw";
    default:
        cerr << "Name of RelaxDirStrategy not known";
        exit_with(utils::ExitCode::SEARCH_UNSUPPORTED);
    }
}
...
std::ostream &operator<<(std::ostream &os, const UCTRewardType &a) {
    ...
    case UCTRewardType::RAND:
        return os << "random";
    default:
        cerr << "Name of AbsMinimizationType not known";
        exit_with(utils::ExitCode::SEARCH_UNSUPPORTED);
    }
}
