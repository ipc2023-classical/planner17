...
void DomainTransitionGraph::read_data(istream &in) {
    ...
    for (size_t origin = 0; origin < nodes.size(); ++origin) {
        ...
        for (int i = 0; i < trans_count; ++i) {
            ...
            if (is_axiom) {
                assert(utils::in_bounds(operator_index, g_axioms));
                the_operator = &g_axioms[operator_index];
            } else {
                assert(utils::in_bounds(operator_index, g_operators));
            ...
            for (size_t j = 0; j < effects.size(); ++j) {
                ...
                int pre = -1;
                utils::HashMap<int, int>::const_iterator pre_it = pre_map.find(var_no);
...
void DomainTransitionGraph::get_successors(int value, vector<int> &result) const {
    assert(result.empty());
    assert(utils::in_bounds(value, nodes));
...
void ValueTransition::simplify_labels(
    ...
    using HashKey = vector<pair<int, int> >;
