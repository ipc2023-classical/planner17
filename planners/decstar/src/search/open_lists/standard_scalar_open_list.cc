...
shared_ptr<OpenList<Entry>> StandardScalarOpenList<Entry>::_parse(OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return make_shared<StandardScalarOpenList<Entry>>(opts);
}
...
StandardScalarOpenList<Entry>::StandardScalarOpenList(
    shared_ptr<Evaluator> eval, bool preferred_only)
