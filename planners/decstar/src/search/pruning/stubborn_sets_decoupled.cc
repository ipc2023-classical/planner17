...
inline vector<pair<int, int>> find_unsatisfied_preconditions(
    const Operator &op, const GlobalState &state, const ExplicitStateCPG *current_cpg) {
    ...
    for (LeafFactorID factor(0); factor < g_leaves.size(); ++factor){
        if (op.has_precondition_on(factor)){
            OpsLeafProps<Condition> preconds = op.get_preconditions(factor);
            ...
            size_t num_leaf_states = current_cpg->get_number_states(factor);
...
inline bool leaf_facts_agree(const LeafState &leaf_state, OpsLeafProps<Effect> leaf_effects) {
    assert(!leaf_effects.empty());
    for (const Effect &eff : leaf_effects){
...
inline bool satisfies_leaf_pre(const LeafState &leaf_state, OpsLeafProps<Condition> leaf_pre) {
    assert(!leaf_pre.empty());
    for (const Condition &pre : leaf_pre){
...
StubbornSetsDecoupled::StubbornSetsDecoupled(const Options &opts) :
    ...
    } else if (g_factoring->get_search_type() == DECOUPLED_SEARCH_TYPE::SAT){
        cout << "Search type \"SAT\" not yet supported by decoupled strong stubborn sets!" << endl;
        exit_with(utils::ExitCode::SEARCH_UNSUPPORTED);
    } else if (g_factoring->get_leaf_representation_type() != LEAF_REPRESENTATION_TYPE::EXPLICIT){
        cout << "Symbolic leaf representation is supported by decoupled strong stubborn sets!" << endl;
        exit_with(utils::ExitCode::SEARCH_UNSUPPORTED);
    }
    cout << "pruning method: stubborn sets decoupled" << endl;

    if (g_factoring->get_search_type() == DECOUPLED_SEARCH_TYPE::UNSAT){
    ...
    } else if (g_factoring->get_profile() == FACTORING_PROFILE::IFORK){
...
void StubbornSetsDecoupled::initialize() {
                ...
                if (!is_fork_leaf[factor] && !is_ifork_leaf[factor] && op.has_effect_on(factor)){
                    ...
                    for (LeafStateHash id(0); id < number_leaf_states; ++id){
                        if (leaf_facts_agree(g_state_registry->lookup_leaf_state(id, factor), op.get_effects(factor))){
                            center_predecessors[factor][id].push_back(op.get_id());
                        }
                    }
                }
            }
    ...
    if (special_case_optimizations && has_fork_leaf){
        ...
        reduced_leaf_state_spaces = ExplicitStateCPG::leaf_state_successors;

        SimulationRelation rel(reduced_leaf_state_spaces, ExplicitStateCPG::leaf_state_predecessors);
        rel.init(10);
        rel.statistics();
        ...
        for (LeafFactorID factor(0); factor < g_leaves.size(); ++factor){
            if (is_fork_leaf[factor] && !g_goals_per_factor[factor].empty()){
                ...
                for (LeafStateHash id(0); id < g_state_registry->size(factor); ++id){
                    // TODO iterate over goal leaf state ids directly
                        ...
                        for (auto &succ : reduced_leaf_state_spaces[factor][id]){
                            if (min_cost_to_goal[factor][succ.second] < numeric_limits<int>::max()){
                                min_cost_to_goal[factor][id] = min(min_cost_to_goal[factor][id],
                                                                   ...
                                                                   get_adjusted_action_cost(g_operators[succ.first], CompliantPathGraph::get_cost_type()));
                            }
                        }
                ...
                if (!is_fork_leaf[factor] && !is_ifork_leaf[factor] && op.has_precondition_on(factor) && op.has_effect_on(factor)){
                    ...
                    for (LeafStateHash id(0); id < number_leaf_states; ++id){
                        ...
                        if (satisfies_leaf_pre(state, op.get_preconditions(factor)) && !leaf_facts_agree(state, op.get_effects(factor))){
                            center_successors[factor][id].push_back(make_pair(op.get_id(), g_state_registry->get_successor_leaf_state_hash(state, op)));
                        }
                    }
                }
            }
...
void StubbornSetsDecoupled::mark_fork_frontier_actions_as_stubborn(LeafFactorID factor) {
    ...
    if (g_goals_per_factor[factor].empty() || current_cpg->get_goal_cost(factor) == CompliantPathGraph::get_min_leaf_goal_cost(factor)){
    ...
    for (LeafStateHash id(0); id < g_state_registry->size(factor); ++id){
        if (current_cpg->has_leaf_state(id, factor)){
            ...
            for (const auto &succ : reduced_leaf_state_spaces[factor][id]){
                ...
                new_price += get_adjusted_action_cost(g_operators[succ.first], CompliantPathGraph::get_cost_type());
    ...
    for (LeafStateHash id(0); id < g_state_registry->size(factor); ++id){
        if (current_cpg->has_leaf_state(id, factor)){
            ...
            if (special_case_optimizations && is_fork_leaf[factor]){
                ...
                for (const auto &succ : ExplicitStateCPG::leaf_state_successors[factor][id]){
                    int new_price = curr_cost;
                    new_price += get_adjusted_action_cost(g_operators[succ.first], CompliantPathGraph::get_cost_type());
                    if (!current_cpg->has_leaf_state(succ.second, factor) || new_price < current_cpg->get_cost_of_state(succ.second, factor)){
                        mark_as_stubborn(succ.first);
...
void StubbornSetsDecoupled::mark_reached_enabling_set_as_stubborn(LeafFactorID factor, OpsLeafProps<Condition> leaf_pre) {
...
void StubbornSetsDecoupled::compute_interference_relation() {
    // TODO try to compute interference incrementally
    ...
    */
    for (OperatorID op1_no(0); op1_no < num_operators; ++op1_no) {
        ...
        vector<OperatorID> &interfere_op1 = interference_relation[op1_no];
        for (OperatorID op2_no(0); op2_no < num_operators; ++op2_no) {
            if (op1_no == op2_no) {
                continue;
            }
...
bool StubbornSetsDecoupled::mark_as_stubborn(OperatorID op_no) {
    if (!special_case_optimizations || !check_ifork_applicable || !has_ifork_leaf){
        ...
        for (LeafFactorID factor(0); factor < g_leaves.size(); ++factor){
            if (is_ifork_leaf[factor] && op.has_precondition_on(factor)){
                bool reached = false;
                OpsLeafProps<Condition> pre = op.get_preconditions(factor);
                size_t num_leaf_states = current_cpg->get_number_states(factor);
                for (LeafStateHash id(0); id < g_state_registry->size(factor); ++id){
                    if (current_cpg->has_leaf_state(id, factor)){
                        if (satisfies_leaf_pre(g_state_registry->lookup_leaf_state(id, factor), pre)){
                            reached = true;
                            break;
                        }
...
void StubbornSetsDecoupled::initialize_stubborn_set(const GlobalState &state) {
    current_cpg = static_cast<const ExplicitStateCPG*>(CPGStorage::storage->get_cpg(state));
    ...
    for (LeafFactorID factor(0); factor < g_leaves.size(); ++factor){
        if (!g_goals_per_factor[factor].empty() && !current_cpg->goal_reachable(factor)){
            if (goal_ingoing_transitions){
                ...
                for (LeafStateHash id : ExplicitStateCPG::leaf_goal_states[factor]){
                    assert(!current_cpg->has_leaf_state(id, factor));
                    for (auto &pred : ExplicitStateCPG::leaf_state_predecessors[factor][id]){
                        mark_as_stubborn(pred.first);
                    }
                    if (!is_fork_leaf[factor] && !is_ifork_leaf[factor] && !center_predecessors[factor].empty()){
                        for (OperatorID op_no : center_predecessors[factor][id]){
                ...
                for (LeafStateHash id(0); id < g_state_registry->size(factor); ++id){
                    if (current_cpg->has_leaf_state(id, factor)){
                        assert(!ExplicitStateCPG::is_leaf_goal_state(id, factor));
                        const LeafState &leaf_state = g_state_registry->lookup_leaf_state(id, factor);
        ...
        if (!g_goals_per_factor[factor].empty()){
            ...
            if (!special_case_optimizations){
                ...
                if (is_fork_leaf[factor]){
                    if (current_cpg->get_goal_cost(factor) > CompliantPathGraph::get_min_leaf_goal_cost(factor)){
                        mark_goal_frontier_actions_as_stubborn(factor);
                    }
                } else if (!is_ifork_leaf[factor] && (leaf_part_g + current_cpg->get_goal_cost(factor)) > CompliantPathGraph::get_min_leaf_goal_cost(factor)) {
                    mark_goal_frontier_actions_as_stubborn(factor);
                }
            }
        }
...
void StubbornSetsDecoupled::handle_stubborn_operator(const GlobalState &state,
                                                     OperatorID op_no) {
    const Operator &op = g_operators[op_no];
        ...
        for (LeafFactorID factor(0); factor < g_leaves.size(); ++factor){
            if (!special_case_optimizations || (!is_fork_leaf[factor] && !is_single_var_ifork_factoring)){
                ...
                if (op.has_precondition_on(factor) || is_single_var_ifork_factoring){
                    ...
                    // otherwise, we loose completeness (see complicated example in JAIR'19 paper notes).
                    mark_fork_frontier_actions_as_stubborn(factor);
                }
            }
        }
...
static shared_ptr<PruningMethod> _parse(OptionParser &parser) {
    ...
    return make_shared<StubbornSetsDecoupled>(parser.parse());
}
