...
bool StubbornSets::can_disable(OperatorID op1_no, OperatorID op2_no) {
...
bool StubbornSets::can_conflict(OperatorID op1_no, OperatorID op2_no) {
    return contain_conflicting_fact(sorted_op_effects[op1_no],
...
void StubbornSets::compute_achievers() {
    ...
    for (int domain_size : g_variable_domain) {
        achievers.push_back(vector<vector<OperatorID>>(domain_size));
    }

    for (size_t op_no = 0; op_no < g_operators.size(); ++op_no) {
        const Operator &op = g_operators[op_no];
        for (const Effect &effect : op.get_effects()) {
            achievers[effect.var][effect.val].push_back(OperatorID(op_no));
        }
    }
}

bool StubbornSets::mark_as_stubborn(OperatorID op_no) {
    if (!stubborn[op_no]) {
        stubborn[op_no] = true;
...
void StubbornSets::prune_operators(
    const GlobalState &state, vector<OperatorID> &ops) {
    if (disabled_pruning){
        return;
